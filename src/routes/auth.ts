...
export const post = async ({ request }: any) => {
    ...
    if (type == 'logout') {
        ...
        return {
            ...
            body: {
                ...
                'Set-Cookie': cookie.serialize('userid', cookie.parse(request.headers.get('cookie') || '').userid, {
                    path: '/',
                    httpOnly: true,
                    maxAge: -1
                }),
    ...
    if (type == 'signup') {
        ...
        await redis.set(cookieId, JSON.stringify({
            email
        }), 'ex', TOKEN_EXPIRE_TIME)
    ...
    if (type == 'login') {
        ...
        if (await redis.get(cookie.parse(request.headers.get('cookie') || '').userid)) {
            ...
            await redis.set(cookie.parse(request.headers.get('cookie') || '').userid, JSON.stringify({
                email
            }), 'ex', TOKEN_EXPIRE_TIME)
            return {
                status: 200,
